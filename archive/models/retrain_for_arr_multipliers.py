#!/usr/bin/env python3
"""
Retrain the model to predict ARR multipliers (ARR_future / ARR_current).
This should be more stable and predictable than growth rates.
"""
...
def create_arr_multiplier_targets(df, horizon=4):
    """Creates ARR multiplier targets for multi-step forecast."""
    print("Step 3: Creating ARR multiplier targets...")
    df_target = df.copy()
    df_target = df_target.sort_values(by=['id_company', 'time_idx'])
    
    # Create targets for future quarters (ARR_future / ARR_current)
    for i in range(1, horizon + 1):
        future_arr = df_target.groupby('id_company')['cARR'].shift(-i)
        current_arr = df_target['cARR']
        df_target[f'Target_Q{i}'] = future_arr / current_arr
    
    print("✅ ARR multiplier target creation complete.")
    return df_target

def main():
    """Main training function."""
    print("🚀 RETRAINING MODEL TO PREDICT ARR MULTIPLIERS")
    print("=" * 60)
    ...
    df_model_ready = create_arr_multiplier_targets(df_featured, horizon=4)
    ...
    # Save the model
    import pickle
    with open('lightgbm_financial_model_arr_multipliers.pkl', 'wb') as f:
        pickle.dump({
    ...
    print(f"\n✅ Model saved to lightgbm_financial_model_arr_multipliers.pkl")
    ...
    print("\n✅ Model retraining for ARR multiplier prediction complete!")
