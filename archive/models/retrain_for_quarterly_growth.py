#!/usr/bin/env python3
"""
Retrain the model to predict quarterly growth rates directly.
This eliminates the need for YoY to quarterly conversion and gives users exactly what they want.
"""
...
def create_quarterly_growth_targets(df, horizon=4):
    """Creates quarterly growth rate targets for multi-step forecast."""
    print("Step 3: Creating quarterly growth rate targets...")
    df_target = df.copy()
    df_target = df_target.sort_values(by=['id_company', 'time_idx'])
    
    # Calculate quarterly growth rates
    df_target['QoQ_Growth'] = df_target.groupby('id_company')['cARR'].pct_change() * 100
    
    # Create targets for future quarters
    for i in range(1, horizon + 1):
        df_target[f'Target_Q{i}'] = df_target.groupby('id_company')['QoQ_Growth'].shift(-i)
    
    print("✅ Quarterly growth target creation complete.")
    return df_target

def main():
    """Main training function."""
    print("🚀 RETRAINING MODEL TO PREDICT QUARTERLY GROWTH RATES DIRECTLY")
    print("=" * 70)
    ...
    df_model_ready = create_quarterly_growth_targets(df_featured, horizon=4)
    ...
    non_feature_cols = [
        ...
        'cARR', 'QoQ_Growth', 'id', 'id_currency', 'id_sector', 
    ...
    # Save the model
    import pickle
    with open('lightgbm_financial_model_quarterly_growth.pkl', 'wb') as f:
        pickle.dump({
    ...
    print(f"\n✅ Model saved to lightgbm_financial_model_quarterly_growth.pkl")
    ...
    print("\n✅ Model retraining for quarterly growth prediction complete!")
