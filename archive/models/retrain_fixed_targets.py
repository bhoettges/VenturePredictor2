#!/usr/bin/env python3
"""
Retrain the model with fixed target variables to eliminate Q1 bias.
Instead of predicting YoY growth for each quarter, predict quarterly growth rates directly.
"""
...
def create_fixed_targets(df, horizon=4):
    """Create fixed target variables that don't have Q1 bias."""
    print("Step 3: Creating fixed target variables...")
    df_target = df.copy()
    df_target = df_target.sort_values(by=['id_company', 'time_idx'])
    
    # Calculate quarterly growth rates
    df_target['QoQ_Growth'] = df_target.groupby('id_company')['cARR'].pct_change(1) * 100
    
    # Create targets for future quarters (quarterly growth rates)
    for i in range(1, horizon + 1):
        df_target[f'Target_Q{i}'] = df_target.groupby('id_company')['QoQ_Growth'].shift(-i)
    
    print("✅ Fixed target creation complete.")
    return df_target
...
def main():
    """Main training function for fixed target models."""
    print("🚀 TRAINING MODELS WITH FIXED TARGETS (NO Q1 BIAS)")
    print("=" * 60)
    ...
    df_model_ready = create_fixed_targets(df_featured, horizon=4)
    ...
    non_feature_cols = [
        ...
        'id_target_customer', 'id_country', 'id_deal_team', 'QoQ_Growth'
    ] + target_cols
    ...
    with open('lightgbm_fixed_target_models.pkl', 'wb') as f:
        pickle.dump(model_data, f)
    
    print(f"\n✅ All models saved to lightgbm_fixed_target_models.pkl")
    
    # Summary
    print("\n--- 📈 FIXED TARGET MODEL PERFORMANCE ---")
    ...
    print("\n✅ Fixed target model training complete!")
