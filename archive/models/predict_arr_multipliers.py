#!/usr/bin/env python3
"""
Predict ARR multipliers using the ARR multiplier model.
This should provide good accuracy with realistic results.
"""
...
def load_arr_multiplier_model():
    """Load the model trained to predict ARR multipliers."""
    try:
        with open('lightgbm_financial_model_arr_multipliers.pkl', 'rb') as f:
            model_data = pickle.load(f)
        print("✅ Model loaded successfully from lightgbm_financial_model_arr_multipliers.pkl")
        return model_data
    except FileNotFoundError:
        print("❌ ERROR: Model file not found. Please run retrain_for_arr_multipliers.py first.")
        return None
    except Exception as e:
        print(f"❌ ERROR: Failed to load model: {e}")
        return None

def predict_arr_progression_with_multipliers(model_data, company_df):
    """
    Predicts ARR progression using ARR multipliers.
    """
    ...
    # Make predictions - these are ARR multipliers
    predicted_multipliers = model_pipeline.predict(X_predict)[0]
    print(f"🔍 Debug: Model predictions (ARR multipliers) = {predicted_multipliers}")

    # --- Create forecast results ---
    print("Step 3: Creating ARR progression using multipliers...")
    ...
    for i in range(4):
        multiplier = predicted_multipliers[i]
        
        # Apply multiplier to get future ARR
        predicted_arr = current_arr * multiplier
        ...
        print(f"🔍 Q{i+1} Debug: Multiplier = {multiplier:.3f} → ARR = ${predicted_arr:,.0f} | QoQ Growth = {quarterly_growth_pct:.1f}%")
...
def main():
    """Test the ARR multiplier prediction with sample data."""
    print("🧪 TESTING ARR MULTIPLIER PREDICTION")
    print("=" * 50)
    
    # Load the model
    model_data = load_arr_multiplier_model()
    ...
    # Make prediction
    result = predict_arr_progression_with_multipliers(model_data, input_data)
    
    print("\n🎯 PREDICTION RESULTS (ARR MULTIPLIERS):")
    print(result)
